...
TEST_F(rdiReaderTest, ImageInfoSection)
{
  ...
  EXPECT_EQ(string("QuickStudy 201002012255"), rdi_i->image_info().Study_Name()) << "Invalid Study Name";
  EXPECT_EQ(93, rdi_i->image_info().Image_Frames()) << "Invalid Image Frames";
  EXPECT_EQ(10, rdi_i->image_info().Image_Lines()) << "Invalid Image Lines";
...
TEST_F(rdiReaderTest, ImageParametersSection)
{
  ...
  EXPECT_FLOAT_EQ(19., rdi_i->image_parameters().RF_Mode().ActiveProbe().Encoder_Range_Max()) << "Invalid Encoder Range Max";
}
